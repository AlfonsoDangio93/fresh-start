...
function FaqItem({ q, a, isOpen, onToggle }: { q: string; a: string; isOpen: boolean; onToggle: () => void }) {
  ...
  return (
    <div className="border-b border-border last:border-0">
      <button
        onClick={onToggle}
        className="flex items-center justify-between w-full py-5 cursor-pointer text-left group"
        aria-expanded={isOpen}
      >
        <span className={`text-[15px] font-semibold pr-4 transition-colors duration-200 ${isOpen ? "text-primary" : "text-dark group-hover:text-dark/70"}`}>
          {q}
        </span>
        <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 transition-all duration-300 ${isOpen ? "bg-primary rotate-45" : "bg-surface group-hover:bg-primary/10"}`}>
          <svg
            ...
            className={`transition-colors duration-300 ${isOpen ? "text-white" : "text-secondary/40 group-hover:text-primary"}`}
          >
      ...
      >
        <div ref={contentRef}>
          <p className="text-[14px] text-secondary leading-relaxed pb-5 pr-10">
            {a}
          </p>
        </div>
      </div>
